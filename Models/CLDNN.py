...
TRAIN_PATH = 'features/mfcc/split_train'
VAL_PATH = 'features/mfcc/split_validation'
BATCH_SIZE = 128
EPOCHS = 100
...
model.add(Conv2D(256, kernel_size = (5,4), input_shape = (feature_x, feature_y, 1)))
model.add(MaxPooling2D(pool_size = (5, 4), strides = (1,1)))
model.add(BatchNormalization())

for i in range 9:
	model.add(Conv2D(256, kernel_size = (5,4), input_shape = (feature_x, feature_y, 1)), padding = 'same')
	model.add(MaxPooling2D(pool_size = (5, 4), strides = (1,1)), padding = 'same')
	model.add(BatchNormalization())

model.add(Conv2D(256, kernel_size = (3,3), input_shape = (feature_x, feature_y, 1)))
model.add(MaxPooling2D(pool_size = (3, 3), strides = (1,1)))
model.add(BatchNormalization())

for i in range 5:
	model.add(Conv2D(256, kernel_size = (3,3), input_shape = (feature_x, feature_y, 1), padding = 'same'))
...
#model.add(LSTM(832, return_sequences = True))
#model.add(LSTM(832, return_sequences = True))
model.add(Dense(512))
model.add(Activation('relu'))
